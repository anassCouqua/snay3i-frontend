...
for (const slug of INDEXABLE_BLOG_SLUGS) {
  ...
  const byline = `<p class="meta" data-snay3i-byline="1"><strong>${AUTHOR}</strong> · Publié le <time datetime="${DATE_ISO}" data-date-published="1">${DATE_FR}</time> · Mis à jour le <time datetime="${DATE_ISO}" data-date-modified="1">${DATE_FR}</time> · <a href="/about">Méthode éditoriale et à propos</a></p>`;
  ...
  const checks = [
    ...
    ['published date', new RegExp(`data-date-published="1">${DATE_FR}`)],
    ['modified date', new RegExp(`data-date-modified="1">${DATE_FR}`)],
...
console.log(`[authorship] PASS: ${INDEXABLE_BLOG_SLUGS.length} canonical guides have visible publish/modified dates, honest authorship and BlogPosting schema`);
