...
const ensureRoute = (route) => { const file=routeFile(route); if(!fs.existsSync(file)) throw new Error(`[sitemap] curated route does not exist: ${route}`); return file; };
const setRobots = (file, desired) => { let html=fs.readFileSync(file,'utf8'); if(/<meta\s+name=["']robots["'][^>]*>/i.test(html)) html=html.replace(/<meta\s+name=["']robots["'][^>]*>/i,`<meta name="robots" content="${desired}">`); else html=html.replace('</head>',`<meta name="robots" content="${desired}">\n</head>`); fs.writeFileSync(file,html,'utf8'); };
...
for(const root of ['blog','artisan']){
  const base=path.join(publicRoot,root); if(!fs.existsSync(base)) continue;
  for(const entry of fs.readdirSync(base,{withFileTypes:true})){
    if(!entry.isDirectory()) continue;
    const route=`/${root}/${entry.name}`; const file=path.join(base,entry.name,'index.html');
...
const urls=[...new Set(allRoutes)].sort().map(route=>`  <url><loc>${BASE}${route}</loc><changefreq>${changefreq(route)}</changefreq><priority>${priority(route)}</priority></url>`).join('\n');
fs.writeFileSync(output,`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls}\n</urlset>\n`,'utf8');
console.log(`Generated canonical sitemap with ${allRoutes.length} verified routes and ${INDEXABLE_BLOG_SLUGS.length} curated blog guides`);
