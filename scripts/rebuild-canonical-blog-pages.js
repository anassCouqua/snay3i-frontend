...
function visual(slug,title,kind){
  ...
  return `<figure data-snay3i-${block}="1" style="margin:${hero?'0 0 24px':'28px 0'};border-radius:20px;overflow:hidden;background:#0D1B2A;border:1px solid #E8E0D4"><img src="${src}" alt="${esc(label)}" ${hero?'fetchpriority="high"':'loading="lazy"'} width="${hero?'1600':'1200'}" height="${hero?'900':'675'}" style="display:block;width:100%;height:${hero?'280px':'auto'};max-height:${hero?'280':'360'}px;object-fit:cover"><figcaption style="padding:10px 14px;font-size:12px;color:#6f6a64;background:#fff">${esc(label)}</figcaption></figure>`;
}
