...
for (const file of [sourceFile, contentFile, metaFile, expansionContentFile, expansionMetaFile]) {
  if (!fs.existsSync(file)) throw new Error(`[canonical blog] missing ${path.relative(root, file)}`);
}

const editorial = {
  ...JSON.parse(fs.readFileSync(contentFile, 'utf8')),
  ...JSON.parse(fs.readFileSync(expansionContentFile, 'utf8')),
};
const meta = {
  ...JSON.parse(fs.readFileSync(metaFile, 'utf8')),
  ...JSON.parse(fs.readFileSync(expansionMetaFile, 'utf8')),
};
...
// Editorially verified photography mapped by article rather than generic
// category. Every guide gets two distinct photographs plus one Snay3i visual.
const IMAGE_SETS = {
  'trouver-bon-plombier-maroc': {
    hero: U('photo-1749532125405-70950966b0e5'), detail: U('photo-1503789146722-cf137a3c0fea'),
    heroAlt: 'Plombier intervenant sur une installation de salle de bain', detailAlt: 'Clés et outils utilisés pour des travaux de plomberie'
  },
  'tarif-electricien-maroc-2026': {
    hero: U('photo-1758101755915-462eddc23f57'), detail: U('photo-1753272691001-4d68806ac590'),
    heroAlt: 'Électricien testant un tableau électrique avec un multimètre', detailAlt: 'Tableau électrique avec câblage et composants visibles'
  },
  'renovation-maison-maroc-guide': {
    hero: U('photo-1768321902097-1d85e7735c5f'), detail: U('photo-1768321902794-c24fb1f00661'),
    heroAlt: 'Pièce intérieure en cours de rénovation avec cloisons apparentes', detailAlt: 'Chantier de rénovation intérieure avec matériaux et outils'
  },
  'climatisation-maroc-installation': {
    hero: U('photo-1698479603408-1a66a6d9e80f'), detail: U('photo-1726614846573-c1ac2e6161d1'),
    heroAlt: 'Unité extérieure de climatisation résidentielle', detailAlt: 'Équipement de climatisation installé à l’extérieur d’un bâtiment'
  },
  'serrurier-urgence-maroc': {
    hero: U('photo-1750614335370-fa6e54d970db'), detail: U('photo-1654944932733-bca31b703dd7'),
    heroAlt: 'Clés insérées dans la serrure d’une porte', detailAlt: 'Clé et serrure de porte vues de près'
  },
  'choisir-carreleur-maroc': {
    hero: U('photo-1523413307857-ef24c53571ae'), detail: U('photo-1703868669362-562283170216'),
    heroAlt: 'Pose manuelle de carrelage mural pendant des travaux', detailAlt: 'Cuisine finie avec carrelage mural et joints visibles'
  },
  'macon-construction-maroc': {
    hero: U('photo-1727786550996-4fa0512e633b'), detail: U('photo-1764697757348-e3c87b076310'),
    heroAlt: 'Maçon travaillant sur un mur en briques', detailAlt: 'Ouvrier réalisant des travaux de maçonnerie sur un mur'
  },
  'urgence-plomberie-casablanca': {
    hero: U('photo-1746095792963-74106bae8658'), detail: D('c314Gh8dXAo'),
    heroAlt: 'Ouvriers réparant une canalisation pendant une intervention de plomberie', detailAlt: 'Plombier intervenant sous un évier sur les raccords et canalisations'
  },
  'petites-reparations-bricoleur-maison-maroc': {
    hero: U('photo-1504148455328-c376907d081c'), detail: U('photo-1530124566582-a618bc2615dc'),
    heroAlt: 'Bricoleur utilisant des outils pour une petite réparation à la maison', detailAlt: 'Outils manuels organisés pour des réparations et fixations domestiques'
  },
  'repeindre-maison-maroc-guide': {
    hero: U('photo-1589939705384-5185137a7f0f'), detail: U('photo-1562259949-e8e7689d7828'),
    heroAlt: 'Peintre appliquant une nouvelle couleur sur un mur intérieur', detailAlt: 'Rouleau et matériel utilisés pour préparer et peindre un mur'
  },
  'rangement-sur-mesure-menuisier-maroc': {
    hero: D('VMMoVcgTnbA'), detail: D('fw8aQUf14XU'),
    heroAlt: 'Menuisier travaillant le bois dans un atelier de fabrication', detailAlt: 'Menuisier usinant une pièce de bois pour un meuble sur mesure'
  },
  'nettoyage-profond-maison-guide': {
    hero: U('photo-1581578731548-c64695cc6952'), detail: U('photo-1527515637462-cff94eecc1ac'),
    heroAlt: 'Professionnelle réalisant un nettoyage approfondi dans une maison', detailAlt: 'Matériel de nettoyage préparé pour traiter différentes surfaces du logement'
  },
  'creer-beau-jardin-maroc': {
    hero: U('photo-1771502674766-8fcbab194e67'), detail: D('bbTcXmW882s'),
    heroAlt: 'Jardiniers plantant et organisant un nouvel espace paysager', detailAlt: 'Jardinière installant des plantes dans un massif ensoleillé'
  },
  'projet-soudure-ferronnerie-maroc': {
    hero: U('photo-1601119462113-bdc0b695988e'), detail: D('Ws8SCmDS9mw'),
    heroAlt: 'Soudeur équipé travaillant sur une pièce métallique en atelier', detailAlt: 'Soudeur réalisant un assemblage sur une structure métallique'
  }
};
...
function splitBodyAtMidHeading(body){const headings=[...body.matchAll(/<h2\b/gi)];if(headings.length<2)return[body,''];const chosen=headings[Math.max(1,Math.floor(headings.length/2))];return[body.slice(0,chosen.index),body.slice(chosen.index)];}
function photoFigure(slug,kind){
  const set=IMAGE_SETS[slug]; if(!set)throw new Error(`[canonical blog] missing image set: ${slug}`);
...
function diagramFigure(slug,title){
  const inlineFile=path.join(blogImageRoot,`${slug}.svg`); if(!fs.existsSync(inlineFile))throw new Error(`[canonical blog] missing inline artwork: ${slug}`);
...
function htmlFor(a){
  const body=markdownToHtml(a.content); const [bodyA,bodyB]=splitBodyAtMidHeading(body);
  const hero=photoFigure(a.slug,'hero'); const detail=photoFigure(a.slug,'detail'); const diagram=diagramFigure(a.slug,a.title); const imageSet=IMAGE_SETS[a.slug];
