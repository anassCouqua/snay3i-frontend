...
function markdownToHtml(markdown) {
  ...
  for(const raw of lines){
    ...
    const h=line.match(/^##\s+(.+)$/); if(h){flush();close();out.push(`<h2>${inlineMd(esc(h[1]))}</h2>`);continue;}
...
function extractContent(source,slug){
  const marker=`slug: '${slug}'`; const start=source.indexOf(marker);
  if(start<0)throw new Error(`[canonical blog] source article not found: ${slug}`);
