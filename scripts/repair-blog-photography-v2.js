...
const CATEGORY_EMOJI = { plomberie:'🔧', electricite:'⚡', renovation:'🏠', peinture:'🎨', serrurerie:'🔑', climatisation:'❄️', jardinage:'🌿', menuiserie:'🪚', carrelage:'🏛️', ferronnerie:'🔥', ville:'🇲🇦' };

function jsMap() { return JSON.stringify(PHOTO_REGISTRY, null, 2); }
...
function repairReactBlog() {
  ...
  const safeHelper = `function SafeArticlePhoto({ src, alt, compact = false }) {\n  const height = compact ? 150 : 280;\n  return (\n    <div data-snay3i-photo="1" style={{position:'relative',width:'100%',minHeight:height,margin:compact?'0 0 14px':'24px 0',borderRadius:16,overflow:'hidden',background:'${FALLBACK}',border:'1.5px solid #E8E0D4'}}>\n      {src ? <img src={src} alt={alt || 'Snay3i.ma — artisan au Maroc'} loading={compact?'eager':'lazy'} onError={(e)=>{e.currentTarget.style.display='none';}} style={{display:'block',width:'100%',height,objectFit:'cover'}} /> : null}\n      <div style={{position:'absolute',left:12,bottom:12,background:'rgba(13,27,42,.82)',color:'#fff',padding:'6px 10px',borderRadius:999,fontSize:12,fontWeight:700}}>🇲🇦 Snay3i.ma</div>\n    </div>\n  );\n}\n\n`;
...
function emojiForSlug(slug) { return CATEGORY_EMOJI[categoryForSlug(slug)] || '🇲🇦'; }
...
function replaceCover(html, imageUrl, altText) {
  const block = safePhotoBlock(imageUrl, altText);
  const empty = /<div([^>]*data-snay3i-article-cover=["']1["'][^>]*)>\s*<\/div>/i;
  if (empty.test(html)) return html.replace(empty, `<div$1>${block}</div>`);
  return html;
}
function removeOldInlinePhotos(html) { return html.replace(/<div[^>]*data-snay3i-inline-photo=["']1["'][\s\S]*?<\/div>\s*<\/div>/gi,''); }
function removeOldImageTags(html) { return html.replace(/<img([^>]*src=["'](?:\/)?blog-images\/[^"']+["'][^>]*)>/gi,''); }
function removeDuplicateGeneratedLayers(html) {
  html = html.replace(/\s*<section[^>]*data-flagship-guide=["']1["'][\s\S]*?<\/section>/gi,'');
  html = html.replace(/\s*<section[^>]*class=["'][^"']*flagship-section[^"']*["'][^>]*>[\s\S]*?<\/section>/gi,'');
  html = html.replace(/\s*<section[^>]*data-editorial-quality=["']1["'][\s\S]*?<\/section>/gi,'');
  html = html.replace(/\s*<section[^>]*data-editorial-related=["']1["'][\s\S]*?<\/section>/gi,'');
  return html;
}
function processArticle(filePath) {
  ...
  html = removeDuplicateGeneratedLayers(html);
...
function processBlogIndex() {
  ...
  html = html.replace(/<div class="card">([\s\S]*?)<\/div>/gi,(card)=>{
    const slugMatch = card.match(/href=["']\/blog\/([^"']+)["']/i); if(!slugMatch) return card;
    const slug = normalizeSlug(slugMatch[1]); const imageUrl = photoForSlug(slug); const emoji = emojiForSlug(slug);
    const titleMatch = card.match(/<h3[^>]*>([\s\S]*?)<\/h3>/i); const alt = titleMatch ? titleMatch[1].replace(/<[^>]+>/g,'').trim() : `Guide ${slug}`;
    const image = imageUrl ? `<div data-snay3i-blog-photo="1" role="img" aria-label="${escapeHtml(alt)}" style="height:150px;width:100%;border-radius:14px;margin-bottom:12px;background-color:#0D1B2A;background-image:linear-gradient(135deg,rgba(13,27,42,.10),rgba(27,38,59,.58)),url('${imageUrl}');background-size:cover;background-position:center"></div>` : `<div data-snay3i-blog-photo="1" role="img" aria-label="${escapeHtml(alt)}" style="height:150px;width:100%;border-radius:14px;margin-bottom:12px;background:${FALLBACK};display:flex;align-items:flex-end;padding:12px;box-sizing:border-box"><span style="background:rgba(13,27,42,.82);color:#fff;border-radius:999px;padding:6px 9px;font-size:18px">${emoji}</span></div>`;
    return `<div class="card">\n${image}${card.replace(/^<div class="card">/,'').replace(/<\/div>\s*$/,'')}\n</div>`;
  });
  fs.writeFileSync(filePath,html,'utf8'); return true;
}
function main(){ repairReactBlog(); if(fs.existsSync(publicRoot)){ for(const entry of fs.readdirSync(publicRoot,{withFileTypes:true})){ if(!entry.isDirectory()) continue; const filePath=path.join(publicRoot,entry.name,'index.html'); if(fs.existsSync(filePath)) processArticle(filePath); } processBlogIndex(); } console.log('[blog photography v2] completed: unique registry, duplicate layers removed, article heroes repaired'); }
main();
